    ...
    for (form in T.form.means) {
      for (pop in T.pop.means) {
        ...
        for(node in nodes) {
          ...
          #a<-scan()
          write.table(cqcfixed,"cqcfix.prm",row.names = FALSE,col.names = FALSE)
          mod1<-tam.mml(simResp,irtmodel="PCM2",B=AB2$B,xsi.fixed=xsifixed,beta.fixed=FALSE,control=list( nodes = seq(-6,6,len=node)))
          ...
          AB2<-PcxDesign(sco,resp=simResp)
          mod2<-tam.mml(simResp,irtmodel="PCM2",B=AB2$B,xsi.fixed=xsifixed,beta.fixed=FALSE,group=c(rep(1,sizeT),rep(2,sizeA)),control=list( nodes = seq(-6,6,len=node)))
...
#simulation(1000,1000,c(1000),c(-1),c(1),c(15),seed=1)
